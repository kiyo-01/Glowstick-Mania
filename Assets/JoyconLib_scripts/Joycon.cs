...
public class Joycon
{
    ...
    {
        return new Vector3(-gyr_g.y, gyr_g.z, -gyr_g.x);
    }
    public Vector3 GetAccel()
    {
        return new Vector3(acc_g.y, -acc_g.z, acc_g.x);
    }
    public Quaternion GetOrientation()
    {
        Vector3 forward = new Vector3(j_b.x, -k_b.x, i_b.x);
        Vector3 up = new Vector3(-j_b.z, k_b.z, -i_b.z);
    ...
    public void Update()
    {
        if (state > state_.NO_JOYCONS)
        {
            ...
            while (reports.Count > 0)
            {
                ...
                ts_de = report_buf[1];
                DebugPrint($"Dequeue. Queue length: {reports.Count}. Packet ID: {report_buf[0]}. Timestamp: {report_buf[1]}. Lag to dequeue: {System.DateTime.Now.Subtract(rep.GetTime())}. Lag between packets (expect 15ms): {rep.GetTime().Subtract(ts_prev)}",
                    DebugType.THREADING);
                ts_prev = rep.GetTime();
            }
